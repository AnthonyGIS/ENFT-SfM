...
void FeatureTracker::Initialize(const Sequence &seq, const Configurator &param) {
	...
	const ushort searchRangeKeyToCurrent = m_bruteForceMatching ? USHRT_MAX :ushort(param.GetArgument("sift_matching_search_range_key_to_current_frame",USHRT_MAX));
	const ushort searchRangeLastToCurrent = m_bruteForceMatching ? USHRT_MAX :ushort(param.GetArgument("sift_matching_search_range_last_to_current_frame",USHRT_MAX));
	const float descDotTh =param.GetArgument("sift_matching_descriptor_dot_product_threshold", 0.95f);
	const float nearest1To2RatioTh =param.GetArgument("sift_matching_descriptor_nearest_first_to_second_ratio_threshold", 0.7f);
	...
	m_flowPositiveVotingTh = ushort(param.GetArgument("flow_positive_voting_threshold", 10));
	m_flowPositiveVotingRatioTh = param.GetArgument("flow_positive_voting_ratio_threshold", 0.5f);
	...
	m_bfPositiveVotingRatioTh = param.GetArgument("bf_positive_voting_ratio_threshold", 0.8f);
...
void FeatureTracker::Run(Sequence &seq, const std::string outputFileName, FILE *fpTiming) 
{

	if (outputFileName != "" && seq.LoadBwithDir(outputFileName.c_str()))
		return;

	const MeasurementIndex nMeasurementMax = MeasurementIndex(m_siftMaxNumFtrs*seq.GetFramesNumberTotal());
	...
	if (m_bruteForceMatching)
		...
		RunSequentialTracking(seq, fpTiming); // 开始序列形式的特征点追踪
